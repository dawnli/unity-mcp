...
def test_should_start_http_server_treats_upm_patch_version_as_four_part_revision(monkeypatch):
    stopped = []
    monkeypatch.setattr(
        main,
        "_read_existing_http_server_health",
    ...
    assert main._should_start_http_server("127.0.0.1", 8080, "9.6.8-patch.2") is True
    assert stopped == [(8080, "9.6.9")]
