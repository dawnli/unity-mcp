...
- MCP clients keep one fixed HTTP endpoint: `http://127.0.0.1:8080/mcp`.
- The target project is selected per call by passing `unity_instance="<hash>"` on Unity tools, or `?unity_instance=<hash>` on resource URIs.
- The hash is computed from the absolute Unity project root path after normalizing it:
  - Convert `\` to `/`
